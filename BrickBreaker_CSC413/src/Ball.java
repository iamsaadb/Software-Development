...
public class Ball {
	...
	void move() {
		...
		if (x + xa <= 0)
			...
			if (game.racket.lives == 0)
				game.gameOver();
			else if (game.racket.lives > 0) {
				Racket.looseLive(game);
			}
		}
		// Collision with bar
		else if (collision()){
			ya = -1;
			y = game.racket.getTopY() - DIAMETER + 10;
		}
	...
	private boolean collision() {
		if (game.racket.getBounds().intersects(getBounds())) {
			return true;
		}
		else if (game.racket.getLeftBounds().intersects(getBounds())) {
			if (xa > 0) {
				xa *= -1;
				if (x*xa < 0 && xa+1 != 0)
					xa++;
		...
		else if (game.racket.getRightBounds().intersects(getBounds())) {
			if (xa < 0) {
				xa *= -1;
				if (x*xa > 0 && xa-1 != 0)
					xa--;
		...
		for (int i = 0; i < game.brick.bricks.size(); i++) {
			if (game.brick.bricks.get(i).getBounds().intersects(getBounds())) {
				...
				if (game.brick.bricks.get(i).hasReward()) {
					game.rewards.createReward(game.brick.bricks.get(i).typeReward
                                                                 , game.brick.bricks.get(i).x-3
                                                                 , game.brick.bricks.get(i).y);
				}
