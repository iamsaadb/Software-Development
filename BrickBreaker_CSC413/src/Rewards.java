...
public class Rewards {
	...
	public Rewards(BrickBreaker game) {
		this.game = game;
	}
	...
	public void createAllRewards() {
	    ...
	    while (reward_ultraball_num > 0) {
			...
			if (game.brick.bricks.get(random.num).typeReward == "") {
				game.brick.bricks.get(random.num).typeReward = "UltraBall";
	    ...
	    while (reward_bigball_num > 0) {
	    	Random random = new Random();
    		if (game.brick.bricks.get(random.num).typeReward == "") {
    			game.brick.bricks.get(random.num).typeReward = "BigBall";
    	    	reward_bigball_num--;
    		}
	    }
	    while (reward_smallball_num > 0) {
	    	Random random = new Random();
    		if (game.brick.bricks.get(random.num).typeReward == "") {
    			game.brick.bricks.get(random.num).typeReward = "SmallBall";
	    ...
	    while (reward_bigbar_num > 0) {
	    	Random random = new Random();
    		if (game.brick.bricks.get(random.num).typeReward == "") {
    			game.brick.bricks.get(random.num).typeReward = "BigBar";
    	    	reward_bigbar_num--;
    		}
	    }
	    while (reward_smallbar_num > 0) {
	    	Random random = new Random();
    		if (game.brick.bricks.get(random.num).typeReward == "") {
    			game.brick.bricks.get(random.num).typeReward = "SmallBar";
    	    	reward_smallbar_num--;
    		}
	    }
	    while (reward_extralive_num > 0) {
	    	Random random = new Random();
    		if (game.brick.bricks.get(random.num).typeReward == "") {
    			game.brick.bricks.get(random.num).typeReward = "ExtraLive";
		...
		for (int i = 0; i < current_rewards.size(); i++) {
			if (current_rewards.get(i).type == "UltraBall" && type == current_rewards.get(i).type) {
				...
				if (Racket.WIDTH == 15) {
					Racket.WIDTH += 15;
					Racket.SIDE_WIDTH += 5;
					Racket.racketMainColor = Color.WHITE;
					Racket.racketSideColor = Color.GRAY;
				}
				else {
					Racket.WIDTH += 15;
					Racket.racketMainColor = Color.CYAN;
					Racket.racketSideColor = Color.BLUE;
				}
			}
				...
				else if (Racket.WIDTH == 45) {
					Racket.WIDTH -= 15;
					Racket.racketMainColor = Color.WHITE;
					Racket.racketSideColor = Color.GRAY;
				}
				else if (Racket.WIDTH > 15) {
					Racket.WIDTH -= 15;
					Racket.racketMainColor = Color.CYAN;
					Racket.racketSideColor = Color.BLUE;
				}
	...
	public void paint(Graphics2D g) {
		for (int i = 0; i < current_rewards.size(); i++) {
			if (current_rewards.get(i).reward_brick_on) {
				if (!racketCollision(i)) {
					paintBrickReward(g);
					if (current_rewards.get(i).y == BrickBreaker.HEIGHT) {
						current_rewards.remove(current_rewards.get(i));
					}
					else {
	...
	private boolean racketCollision(int i) {
		if (game.racket.getBounds().intersects(getBounds(i))) {
			return true;
		}
		else if (game.racket.getLeftBounds().intersects(getBounds(i))) {
			return true;
		}
		else if (game.racket.getRightBounds().intersects(getBounds(i))) {
			return true;
		}
		else {
			return false;
